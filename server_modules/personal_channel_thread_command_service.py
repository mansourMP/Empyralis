...
def build_thread_command_reply(
    ...
    *,
    ...
    if command.action == "start_new_thread":
        ...
        return f"Thread switch requested: start {title}. Empyralis will persist this as a new agent thread."
    if command.action == "list_threads":
        rows = _thread_rows(available_threads)
        if not rows:
            return "No agent threads are available for this messaging lane yet."
        return "Available agent threads:\n" + "\n".join(rows)
    ...
    if command.action == "invalid_thread_reference":
        return "Use `/use <thread-id>` with a valid agent thread id."
    if command.action == "show_status":
        active = active_thread_id.strip() or "default"
        return f"This messaging lane is mapped to agent thread `{active}`."
