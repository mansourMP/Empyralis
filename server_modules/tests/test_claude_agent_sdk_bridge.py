...
class TurnEngineSelectionFlagOffTests(unittest.TestCase):
    """Pins that _run_sage_action_loop_v3's engine flag resolves correctly.
    SDK is the production default; under pytest, legacy is the default so
    existing test mocks continue to work."""

    def test_none_defaults_to_legacy_under_pytest(self):
        self.assertNotEqual(
            sage_agent_runtime_service._resolve_turn_engine_id(None),
            claude_agent_sdk_bridge.ENGINE_ID,
        )

    def test_empty_dict_defaults_to_legacy_under_pytest(self):
        self.assertNotEqual(
            sage_agent_runtime_service._resolve_turn_engine_id({}),
            claude_agent_sdk_bridge.ENGINE_ID,
        )

    def test_non_dict_defaults_to_legacy_under_pytest(self):
        self.assertNotEqual(
