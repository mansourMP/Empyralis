...
- **A port is NOT a cookie jar.** Cookies are scoped by host, never by port (RFC 6265), and `_cookie_domain()` returns None locally, so every e2e stack on `127.0.0.1:<anything>` shares ONE jar: any agent's login/logout/refresh silently clobbers every other agent's session. Symptom is a browser dying in 15–180s with `"Authentication required."` while `curl` on the same backend runs clean for minutes — read as a catastrophic auth bug three times on 2026-08-31, and it is not one. Bind to a unique `*.localhost` host, and confirm `document.cookie` is empty before the first login.
...
- **Every agent is the same agent underneath.** A job (Bookkeeping, Marketing…) may differ only in words, audience and skills — never in capability. All jobs carry `capability_preset: standard`; the `knowledge` preset is never offered by the picker because its hardware lock is permanent. Guards: `agent-create-job.test.ts`, `agent-quick-create.test.ts`.
...
- **Channels: Telegram (recommended), Slack, and Discord.** All three work with no paired hardware. Discord needs a standing per-customer WebSocket — accepted cost, not a blocker (founder, 2026-08-30, after the code was found already running it at every boot: "Discord is going to stay").
...
- Agents has two layouts behind one view-options popover — **List (default) · Board** — Cards is gone (founder reversal, 2026-08-30: *"i do not want cards thing default should be list ... i only want to see list and board!"*). **A card/row face is still two facts and refuses a third**: state (stopped/blocked/working/idle) and reach (the task it is on > tasks waiting > where it answers > neither), and both surfaces draw the SAME reach line via `agentCardReach` (agent-card-face.ts, kept — it was never the Cards component itself). `activity_preview` is a lifecycle verb, true of every agent, and putting it back as the secondary line is the defect this surface has regressed to twice. List/Board sort by the reader's own Ordering choice, never by attention rank — that ranking is `planAgentCards`'s own filter-only rule now, since no renderer uses its sort order any more. Guard: `agent-view-options.test.ts`.
...
- **`agent_traces` needs exactly one finisher per row.** `finish_agent_trace` has no `WHERE finished_at IS NULL` guard — a second call silently overwrites the first outcome — and `prune_finished_agent_traces` deliberately never reaps a row with `finished_at IS NULL`, so a trace nobody finishes accumulates forever instead of failing loud. `agent_turn.py`'s `surface=web` trace was exactly this (250/250 rows, `finished_at` NULL, fixed 2026-08-30: `agent_turn()` opened it and handed it to a not-yet-run SSE producer that never saw it again — `_run_sage_action_loop_v3` opened a second, unrelated `surface=sage` trace for the real work instead). Still open: that `surface=sage` trace's SDK-engine branch has its own gaps (an exception inside `_collect_stream_events`'s `asyncio.to_thread` skips `_finish_sdk_engine_trace` entirely) — 46/72 sampled rows were still unfinished after two rounds of fixes. The honest full repair is one trace per turn, not two; not done here.
