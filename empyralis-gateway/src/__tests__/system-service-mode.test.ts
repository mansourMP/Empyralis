...
test("explicit desktop permission denial removes native action capabilities", async () => {
  await withServiceModeEnv({
    ...
    const snapshot = await collectPassiveInventorySnapshot({
      // memory.write has no desktop-permission mapping at all (not_applicable,
      // always ready) — used here as the "unrelated capability" control.
      // shell.execute is deliberately NOT used for that role anymore: it now
      // maps to the shell_sandbox permission (Docker-gated), so it would be
      // "restricted" in this test's own docker-unavailable mock (commandExists
      // returns null for everything, including "docker") — that's exercised
      // directly in service-inventory.test.ts instead.
      requestedCapabilities: ["memory.write", "computer_control.click", "browser.session.start"],
      deps: {
        env: {
          EMPYRALIS_AGENT_COMPUTER_PERMISSION_ACCESSIBILITY: "denied",
          EMPYRALIS_AGENT_COMPUTER_PERMISSION_BROWSER: "restricted",
    ...
    assert.deepEqual(snapshot.capability_readiness.ready, ["memory.write"]);
