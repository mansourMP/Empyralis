...
| `/w/:workspaceId/channels` | Channel-related Sage surface or connector pane | Personal-channel/gateway calls are route-backed but not clearly unified in shell | `/api/personal-channels/gateways/{gateway_id}/channels`, Telegram status/setup/message routes | Gateway personal-channel state and Telegram channel message state | Missing clear first-class shell component mapping for channels; needs explicit pane or tab link from Agent Computer status. **WhatsApp is not a supported channel — do not include WhatsApp setup UI.** | Pair a gateway, open Channels, verify Telegram (hosted bot) state, send test message only after approval behavior is clear |
